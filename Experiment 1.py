...
# =====================================================================
    ...
    def __call__(self, model, n_tc, gen):
        ...
        # Energy on the uniform FD grid (needed for the FD spatial stencil).
...
def plot_all(hist, ev):
    ...
    for frac, c in [(0.0, '#3182bd'), (0.5, '#e6550d'), (1.0, '#31a354')]:
        it = int(frac * (len(t) - 1))
        ax[0].plot(x, ev['P_ref'][it], color=c, lw=2, label=f'ref t={t[it]:.2f}')
        ax[0].plot(x, ev['P_pred'][it], color=c, lw=1.1, ls='--')
    ax[0].set_xlabel('x'); ax[0].set_ylabel('p(x,t)')
    ax[0].set_title(f'Density snapshots (solid ref, dashed PINN), alpha={a}')
